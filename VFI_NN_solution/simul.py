...
def simulation(params, nn, T, init=None, init_dist=None, last_dist=True):
    ...
    with tqdm(total=T, desc="Simulation Progress", unit="period") as pbar:
        for t in range(T):
            ...
            pnew, alpha, threshold, k_prime_adj = bisectp(nn, params, basic_s, max_expansions=10, max_bisect_iters=30, init=init)
...
def init_next_e0_sim(data, price, nn, params, device):
    ...
    with torch.no_grad():
        ...
        # Multiply to get joint probabilities => (i_size, 5, 5)
