...
class PolicyTrainer():
    ...
    def sampler(self, batch_size, init=None, update_init=False):
        if init is None:
            self.policy_ds = self.init_ds.get_policydataset(self.current_policy, "nn_share", self.price_fn, init, update_init)
...
class KTPolicyTrainer(PolicyTrainer):
    def __init__(self, vtrainers, init_ds, policy_path=None):
        ...
        self.price_loss_training_loop(self.n_sample_price, self.price_config["T"], self.mparam, init_ds.policy_init_only, "nn_share", self.price_model, self.optimizer_price,batch_size=128, init=True, state_init=None, shocks=None, num_epochs= 5) #self.price_config["T"]
